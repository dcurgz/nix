...
in
{
  flake.modules.home-manager.firefox = flake.lib.home-manager.mkAspect (with flake.tags; [ nixos-desktop darwin-desktop])
  ({
    lib,
    config,
    pkgs,
  ...
  {
    config = mkFirefox pkgs.system pkgs;
  });
}
